...
class WritersDesk:
    ...
    def _build_layout(self) -> None:
        ...
        toolbar = ttk.Frame(self.master, padding=(12, 10))
        ...
        ttk.Button(toolbar, text="Save", command=self.save_file).grid(row=0, column=1, padx=(0, 20))
        ttk.Button(toolbar, text="Undo", command=self._undo).grid(row=0, column=2, padx=(0, 6))
        ttk.Button(toolbar, text="Redo", command=self._redo).grid(row=0, column=3, padx=(0, 20))
        ttk.Button(toolbar, text="Dialogue", command=lambda: self._toggle_tag("dialogue")).grid(row=0, column=4, padx=(0, 6))
        ttk.Button(toolbar, text="Narration", command=lambda: self._toggle_tag("narration")).grid(row=0, column=5, padx=(0, 6))
        ttk.Button(toolbar, text="Emphasis", command=lambda: self._toggle_tag("emphasis")).grid(row=0, column=6, padx=(0, 6))
        ttk.Button(toolbar, text="Indent", command=self._indent_selection).grid(row=0, column=7, padx=(0, 6))
        ttk.Button(toolbar, text="Outdent", command=self._outdent_selection).grid(row=0, column=8, padx=(0, 6))
        ttk.Button(toolbar, text="Clear formatting", command=self._clear_formatting).grid(row=0, column=9, padx=(0, 6))

        ttk.Label(toolbar, text="Line spacing:", style="TLabel").grid(row=0, column=10, padx=(12, 4))
        self.line_spacing_var = tk.StringVar(value="0")
        ttk.Entry(toolbar, textvariable=self.line_spacing_var, width=5).grid(row=0, column=11, padx=(0, 6))
        ttk.Button(toolbar, text="Apply", command=self._apply_line_spacing).grid(row=0, column=12, padx=(0, 12))

        ttk.Label(toolbar, text="Font:", style="TLabel").grid(row=0, column=13, padx=(0, 4))
        ...
        self.font_family_combo.grid(row=0, column=14, padx=(0, 4))
        ...
        self.font_size_entry.grid(row=0, column=15, padx=(0, 4))
        ttk.Button(toolbar, text="Set", command=self._apply_font).grid(row=0, column=16, padx=(0, 12))

        ttk.Checkbutton(toolbar, text="Spell", variable=self.spell_enabled, command=self._toggle_spell).grid(row=0, column=17, padx=(6, 0))
        ttk.Checkbutton(toolbar, text="Suggest", variable=self.suggest_enabled, command=self._toggle_suggest).grid(row=0, column=18, padx=(6, 0))
        ttk.Checkbutton(toolbar, text="Grammar", variable=self.grammar_enabled, command=self._toggle_grammar).grid(row=0, column=19, padx=(6, 6))
        ttk.Button(toolbar, text="Check grammar", command=self._manual_grammar_check).grid(row=0, column=20, padx=(0, 12))

        self.word_count_label = ttk.Label(toolbar, text="Words: 0")
        self.word_count_label.grid(row=0, column=21, sticky="e")
        self.total_word_count_label = ttk.Label(toolbar, text="Total: 0")
        self.total_word_count_label.grid(row=0, column=22, sticky="e", padx=(12, 0))
    ...
    def _update_suggestions(self, suggestions: list[str]) -> None:
        if not self.suggest_enabled.get():
            suggestions = []
